...
# -*- coding: utf-8 -*-
"""
video_mask_batch_cuda.py — 视频批量打码（人脸 + 信用卡/银行卡），CUDA 版
...
加速:
  ...
  CUDA / MPS GPU 加速(OWLv2；NVIDIA / Apple Silicon)
  关键帧检测 + 光流跟踪(人脸+卡)
...
FACE_CONF = 0.45              # 人脸置信度阈值(0.5->0.45, 配合时序传播滤误检)
...
class YuNetOnnxRuntimeCuda:
    """使用 ONNX Runtime CUDAExecutionProvider 的 YuNet 推理与后处理。

    YuNet 的 ONNX 输出有 12 个检测头。这里按 OpenCV FaceDetectorYN 的官方
    解码公式还原检测框，因此仅替换推理后端，不改变原有阈值、NMS 和跟踪策略。
    """
    _OUTPUTS = [
    ...
    def __init__(self, model_path):
        ...
        if "CUDAExecutionProvider" not in self.providers:
            raise RuntimeError("ONNX Runtime 未能启用 CUDAExecutionProvider")
        self.input_name = self.session.get_inputs()[0].name
        output_names = {output.name for output in self.session.get_outputs()}
        missing = set(self._OUTPUTS) - output_names
        if missing:
            raise RuntimeError(f"YuNet 输出不兼容: 缺少 {sorted(missing)}")

        # 2023mar 模型通常是固定输入；动态模型则保留输入原始尺寸（补齐到 32）。
    ...
    def _prepare(self, img):
        ...
        if self.fixed_w and self.fixed_h:
            scale = min(self.fixed_w / w0, self.fixed_h / h0)
            resized = cv2.resize(img, (max(1, round(w0 * scale)), max(1, round(h0 * scale))))
            pad = np.zeros((self.fixed_h, self.fixed_w, 3), dtype=np.uint8)
            pad[:resized.shape[0], :resized.shape[1]] = resized
        else:
            scale = 1.0
            pad_w = (w0 + 31) // 32 * 32
            pad_h = (h0 + 31) // 32 * 32
            pad = cv2.copyMakeBorder(img, 0, pad_h - h0, 0, pad_w - w0,
                                     cv2.BORDER_CONSTANT, value=(0, 0, 0))
        blob = np.ascontiguousarray(pad.transpose(2, 0, 1)[None], dtype=np.float32)
        return blob, scale, pad.shape[1], pad.shape[0]

    def detect(self, img, conf):
        blob, scale, pad_w, pad_h = self._prepare(img)
        values = self.session.run(self._OUTPUTS, {self.input_name: blob})
        boxes, scores = [], []
        for pos, stride in enumerate(self._STRIDES):
            cls = np.asarray(values[pos]).reshape(-1)
            obj = np.asarray(values[pos + 3]).reshape(-1)
            bbox = np.asarray(values[pos + 6]).reshape(-1, 4)
            rows, cols = pad_h // stride, pad_w // stride
            count = min(rows * cols, len(cls), len(obj), len(bbox))
            if count == 0:
                continue
            score = np.sqrt(np.clip(cls[:count], 0, 1) * np.clip(obj[:count], 0, 1))
            for index in np.flatnonzero(score >= conf):
        ...
        if not boxes:
            return []
        keep = cv2.dnn.NMSBoxes(boxes, scores, conf, 0.3, top_k=5000)
...
class FaceDetector:
    ...
    def __init__(self, model_dir=None, yunet_size=FACE_YUNET_SIZE):
        ...
        if yunet_path and os.path.exists(yunet_path):
            # 仅在 CUDA provider 实际生效时标记 GPU；失败即回退 OpenCV CPU。
            try:
                self.yunet_ort = YuNetOnnxRuntimeCuda(yunet_path)
                print(f"[人脸] YuNet ONNX Runtime (CUDA GPU; providers={self.yunet_ort.providers})")
            except Exception as exc:
                self.yunet = cv2.FaceDetectorYN_create(
                    yunet_path, "", (320, 320),
                    score_threshold=FACE_CONF, nms_threshold=0.3, top_k=5000)
                print(f"[人脸] YuNet OpenCV (CPU; ONNX Runtime CUDA 未启用: {exc})")
        # 2. res10 SSD 兜底
    ...
    def detect(self, img, conf=FACE_CONF):
        h0, w0 = img.shape[:2]
        if self.yunet_ort is not None:
            # GPU 实现负责固定输入缩放、填充、NMS 与坐标映射。
            return self.yunet_ort.detect(img, conf)
...
class OwlCardDetector:
    ...
    def __init__(self, owl_size=CARD_OWL_SIZE):
        os.environ.setdefault("HF_HUB_OFFLINE", "1")
        ...
        # 优先 CUDA(NVIDIA)，其次 MPS(Apple Silicon)，不可用则回退 CPU。
        # 设备选择放在这个独立文件，避免影响原有算法版本。
        if torch.cuda.is_available():
            self.device = "cuda"
            self.model.to(self.device)
            name = torch.cuda.get_device_name(torch.cuda.current_device())
            print(f"[信用卡] OWLv2 模型已加载 (CUDA GPU: {name})")
    ...
    def get(cls, owl_size=CARD_OWL_SIZE):
        if cls._inst is None or cls._inst.owl_size != owl_size:
            cls._inst = cls(owl_size)
        return cls._inst
...
def find_hw_encoder():
    """检测可用的硬件编码器(macOS VideoToolbox / NVIDIA NVENC / Intel QSV)"""
...
def _process_pipe(src, dst, face_on, card_on, card_detector, card_conf,
    ...
    face_proc = FaceProcessor(fd, detect_int=face_int) if face_on else None
    owl = None
    geo_thr = GEO_SCORE
    if card_on:
        if card_detector == "owlv2":
            try:
                owl = OwlCardDetector.get(owl_size=owl_size)
    ...
    # 输出选项(必须在所有 -i 之后)
    if hw:
        bitrate = max(int(w * h * fps * 0.03), 2000000)
...
def _process_files(src, dst, face_on, card_on, card_detector, card_conf,
    ...
    if hw:
        vcmd += ["-c:v", hw, "-b:v", str(max(int(w * h * fps * 0.03), 2000000)), "-real_time", "0"]
    else:
        vcmd += ["-c:v", "libx264", "-preset", "fast", "-crf", "20"]
...
def process_video(src, dst, face_on=True, card_on=True, card_detector="owlv2",
                  ...
                  use_pipe=False, keep_tmp=False, log=print):
