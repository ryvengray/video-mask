...
class HardFaceRecall:
    ...
    主检测(双模型共识)漏掉的小脸/边缘脸, 先从低分原始候选和单模型
    ...
    def recall(self, detector, img, raw_candidates, existing_dets,
        ...
        """低分原始候选/单模型formal候选困难脸二检, 返回 [(box, roi_conf), ...]。

        raw_candidates: 主检测器产出的原始候选列表。种子有两类:
                        formal=False 的低置信候选(未进入共识), 以及
                        single=True 的单模型formal候选(2.5g组合下没有
                        共识出口, 靠ROI二检补全异构第二意见)。
        ...
        height, width = img.shape[:2]
        recalled = []
        # 收集困难候选并按困难程度排序(最困难优先)
        difficult = []
        for item in raw_candidates:
            is_single_seed = item.get("single", False)
            if item.get("formal", True) and not is_single_seed:
                # 只处理低分候选和单模型formal候选(未进入共识的)
                continue
            box = item["box"]
            # 跳过与已有共识框重叠的(已处理过)
...
class FaceProcessor:
    ...
    def _update_tracks(self, dets, gray, debug_scores=None,
        ...
        for di, det in enumerate(dets):
            ...
            self.tracks.append({
                ...
                "misses": 0,
                "hits": 1,
                # 新建轨迹必来自allow_new检测(S续接allow_new=False),
                # 创建命中即计1次共识级命中
                "consensus_hits": 1,
                "visible": True,
                "visibility_misses": 0,
    ...
    def _find_matching_track(self, box):
        """返回与候选框匹配的轨迹(IoU或中心距门限), 无匹配返回 None。

        匹配门限与单模型续轨一致; 调用方据轨迹 confirmed 状态决定
        单模型框是否具备升级资格。
        """
            ...
            if isinstance(self.detector, DualFaceDetector):
                dual_dets = self.detector.detect_with_scores(
                    ...
                    include_single=(bool(self.tracks) or raw_debug
                                    ...
                                    or self.hard_face_recall is not None),
                ...
                allow_new = [True] * len(dets)
                # stereo低分互证(source=X)与共识同框但证据较弱, 只建pending
                # 轨迹: 真实人脸下一帧二次命中即升级并回补, 瞬态误检被
                # 时间共识拦截, 不再直接打码。
                ...
                if self.hard_face_recall is not None:
                    ...
                    # 出口(单模型新候选被直接拒绝), 不补路会永久漏打;
                    # 已确认轨迹附近的除外(那是S续轨路径)。
                    confirmed_boxes = [track["box"] for track in self.tracks
                                       if track.get("confirmed", True)]
                    for item in single_candidates:
                        if any(self._iou(item["box"], b) >= 0.35
                               for b in confirmed_boxes):
                            continue
                        raw_cands.append({
                            "box": item["box"], "model": "Single",
                            "score": None, "formal": True, "single": True})
                ...
                for item in single_candidates:
                    ...
                    if any(self._iou(box, confirmed["box"]) >= self.dual_iou
                        ...
                        continue
                    matched_track = self._find_matching_track(box)
                    if matched_track is None:
                        continue
                    dets.append(box)
                    debug_scores.append(item["scores"])
                    allow_new.append(False)
                    # S续接只续框不升级: 若目标轨迹是pending, 单模型证据
                    # 不足以升格confirmed, 须等共识级命中(D/V/X/ROI)。
                    s_upgrades = matched_track.get("confirmed", True)
                    new_confirmed.append(s_upgrades)
